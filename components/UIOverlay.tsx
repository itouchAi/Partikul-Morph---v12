    ...
    <>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm" onPointerDown={(e) => e.stopPropagation()}>
          <div className="bg-[#111] border border-white/20 p-6 rounded-2xl max-w-sm w-full shadow-2xl animate-in zoom-in duration-300">
             <div className="flex flex-col items-center gap-4 mb-4">
               <h3 className="text-white font-mono text-lg text-center font-bold tracking-wide">Resim Partikül Modu</h3>
               <div className="w-full h-48 bg-black/50 rounded-lg border border-white/10 overflow-hidden flex items-center justify-center p-2">
                 <img src={pendingImage} alt="Preview" className="w-full h-full object-contain" />
               </div>
             </div>
             
             <div className="mb-6">
                <p className="text-[10px] text-gray-400 mb-2 font-mono uppercase tracking-wider text-center">Partikül Rengi</p>
                <div className="flex bg-black/40 p-1 rounded-lg border border-white/10">
                  <button 
                    onClick={() => setUseOriginalImageColors(true)}
                    className={`flex-1 py-2 rounded-md text-xs font-medium transition-all ${useOriginalImageColors ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'}`}
                  >
                    Orijinal Renkler
                  </button>
                  <button 
             ...
             <div className="flex gap-3">
               <button 
                 ...
                 className="flex-1 py-3 rounded-lg border border-white/10 text-white/50 hover:bg-white/5 hover:text-white transition-colors text-sm"
               >
                 İptal
               </button>
               <button 
                 onClick={confirmImageUpload} 
                 className="flex-1 py-3 rounded-lg bg-green-600 text-white hover:bg-green-500 transition-colors font-bold text-sm shadow-lg shadow-green-900/50"
               >
                 Oluştur
               </button>
             </div>
      ...
      {isSettingsOpen && ( 
        <div className={`absolute top-20 right-6 z-[60] w-80 backdrop-blur-2xl border border-white/20 rounded-3xl p-6 shadow-[0_30px_60px_rgba(0,0,0,0.8)] animate-config-pop origin-top-right cursor-default transition-colors duration-300 ${isLightMode ? 'bg-white/80 text-black border-black/10' : 'bg-[#0a0a0a]/90 text-white border-white/10'}`} onPointerDown={stopProp}> 
            ...
            <div className="space-y-6 max-h-[60vh] overflow-y-auto pr-2 custom-chic-scrollbar" onWheel={(e) => e.stopPropagation()}> 
                {/* Physics Section */}
                <div> 
                    <h5 className={`text-[10px] font-bold uppercase mb-3 opacity-60 flex items-center gap-2 ${isLightMode ? 'text-black' : 'text-white'}`}> <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"></circle><path d="M12 6v6l4 2"></path></svg> Fizik Motoru </h5> 
                    <div className="space-y-4"> 
                        <div className="group"> 
                            <div className="flex justify-between mb-2"> <span className="text-xs font-medium opacity-80">İtme Kuvveti</span> <span className="text-xs font-mono font-bold text-blue-500">%{repulsionStrength}</span> </div> 
                            <StylishSlider min={0} max={100} value={repulsionStrength} onChange={onRepulsionChange} colorClass="bg-blue-500" />
                        </div> 
                        <div className="group"> 
                            <div className="flex justify-between mb-2"> <span className="text-xs font-medium opacity-80">Etki Alanı</span> <span className="text-xs font-mono font-bold text-purple-500">%{repulsionRadius}</span> </div> 
                            <StylishSlider min={10} max={100} value={repulsionRadius} onChange={onRadiusChange} colorClass="bg-purple-500" />
                        </div> 
                    </div> 
                </div> 
                ...
                {/* Structure Section */}
                <div> 
                    <h5 className={`text-[10px] font-bold uppercase mb-3 opacity-60 flex items-center gap-2 ${isLightMode ? 'text-black' : 'text-white'}`}> <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"></path></svg> Partikül Yapısı </h5> 
                    <div className="space-y-4"> 
                        <div className="group"> 
                            <div className="flex justify-between mb-2"> <span className="text-xs font-medium opacity-80">Yoğunluk (Adet)</span> <span className="text-xs font-mono font-bold text-green-500">{(particleCount / 1000).toFixed(1)}k</span> </div> 
                            <StylishSlider min={20000} max={60000} step={1000} value={particleCount} onChange={onParticleCountChange} colorClass="bg-green-500" />
                        </div> 
                        <div className="group"> 
                            <div className="flex justify-between mb-2"> <span className="text-xs font-medium opacity-80">Boyut</span> <span className="text-xs font-mono font-bold text-yellow-500">{particleSize}px</span> </div> 
                            <StylishSlider min={1} max={50} value={particleSize} onChange={onParticleSizeChange} colorClass="bg-yellow-500" />
                        </div> 
                        <div className="group"> 
                            <div className="flex justify-between mb-2"> <span className="text-xs font-medium opacity-80">Form Sıkılığı</span> <span className="text-xs font-mono font-bold text-orange-500">%{modelDensity}</span> </div> 
                ...
                <div>
                  ...
                  {hasImage && ( 
                    <div className="group mb-4"> 
                      ...
                      <StylishSlider min={0} max={20} value={depthIntensity} onChange={onDepthChange} colorClass="bg-red-500" />
                    </div> 
                  )}
        ...
        <div className={`pointer-events-auto w-full max-w-lg relative flex gap-2 items-center transition-transform duration-500 ${hideBottomClass}`} onPointerDown={stopProp}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setIsPaletteOpen(!isPaletteOpen)} className={`w-10 h-10 flex-shrink-0 rounded-full flex items-center justify-center transition-all duration-300 border-2 z-20 group ${isLightMode ? 'bg-black/5 hover:bg-black/20 border-black/20 hover:border-black shadow-[0_0_15px_rgba(0,0,0,0.1)] hover:shadow-[0_0_20px_rgba(0,0,0,0.3)]' : 'bg-white/5 hover:bg-white/20 border-white/20 hover:border-white shadow-[0_0_15px_rgba(255,255,255,0.1)] hover:shadow-[0_0_20px_rgba(255,255,255,0.3)]'}`} title="Renk Paletini Aç" onMouseEnter={onInteractionStart} onMouseLeave={onInteractionEnd}>
                <div className={`w-6 h-6 rounded-full shadow-sm icon-animate-pulse ${isLightMode ? 'border-black/10' : 'border-white/10'}`} style={{ backgroundColor: isOriginalColors && hasImage ? 'transparent' : currentColor, backgroundImage: isOriginalColors && hasImage ? 'conic-gradient(red, yellow, lime, aqua, blue, magenta, red)' : 'none' }}></div>
            </button>
          </div>
        </div>
      </div>
